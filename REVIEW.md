...
> **Update 2026-05-06 — implementation pass complete.** 18 of 20 action items below have been shipped on branch `review-implementation` (5 commits, +1,554 / −506 lines, 67 tests up from 19). See the annotated [Prioritized Action Plan](#prioritized-action-plan) for status markers and the [Post-Implementation Notes](#post-implementation-notes) for three pre-existing bugs the new fixture tests caught and one finding that turned out to be wrong on closer inspection.
...
> **Status legend:** ✅ shipped · ⏭️ deferred (your call) · ❌ withdrawn after closer inspection.

### Immediate (High - This Sprint)

1. ✅ **escapeXml hardened** — moved to [src/ooxml/escape.ts](src/ooxml/escape.ts); now escapes all five XML metacharacters (`&<>'"`).
2. ⏭️ **Dependabot backlog** — out of my scope. 16 PRs still sitting on the remote (your auth and decisions). Merging or closing them is a `gh` operation requiring your call.
3. ✅ **Default scopes tightened** — [src/config.ts:22-51](src/config.ts:22) defaults to read-only scopes; write scopes attach automatically when `MCP_ENABLE_WRITES` (or any per-surface flag) is set, matching the writeGuard's default-deny posture. Active scope set is logged at startup ([src/server.ts:43-51](src/server.ts:43)).
4. ✅ **`docs/tools.md` written** — [docs/tools.md](docs/tools.md) backfills the previously-broken link with the full 92-tool catalogue grouped by surface, scopes per tool.
5. ✅ **`--version` flag** — [src/config.ts:9-17](src/config.ts:9) reads from `package.json` so there's a single source of truth. Verified `node dist/index.js --version` → `0.1.0`.

### Short-term (Medium - Next 2-4 Sprints)

6. ✅ **OOXML fixture-test suite** — [test/ooxml.test.ts](test/ooxml.test.ts), 26 tests. Fixtures generated at runtime via `docx` and `pptxgenjs` rather than committed binaries; round-trips every mutator through PizZip.
7. ✅ **`src/ooxml/` shared module** — pure functions in [src/ooxml/escape.ts](src/ooxml/escape.ts), [src/ooxml/docx.ts](src/ooxml/docx.ts), [src/ooxml/pptx.ts](src/ooxml/pptx.ts). Word surface shrank from 410 → 245 lines; PowerPoint from 449 → 235.
8. ✅ **Integration tests for one read + one write per surface** — [test/integration.test.ts](test/integration.test.ts), 22 tests. Used a fake-Graph recorder rather than nock since the goal is URL/payload assertions and Node 20+'s `undici` makes nock awkward.
9. ✅ **`npm audit` step in CI** — [.github/workflows/ci.yml:25-28](.github/workflows/ci.yml:25), `--omit=dev --audit-level=high`, `continue-on-error: true` until the Dependabot queue is clean.
10. ✅ **Path validation** — [src/util/schema.ts:14-43](src/util/schema.ts:14) defines `DrivePath` and `Filename` Zod schemas; applied to files / excel / word / powerpoint. Integration test asserts `..`-segment rejection.
11. ✅ **`.env.example`** — [.env.example](.env.example) covers every `MCP_*` var with inline comments.
12. ✅ **`--list-tools` and `--logout` flags** — [src/cli.ts](src/cli.ts) plus dispatch in [src/index.ts](src/index.ts). `--list-tools` shows the catalogue with visibility markers under the current config; `--logout` deletes the token cache (ENOENT treated as already-logged-out).

### Long-term (Low/Info - Backlog)

13. ✅ **Coverage threshold** — [vitest.config.ts:8-19](vitest.config.ts:8) gates 50% lines/functions/statements and 40% branches over the orchestration + ooxml surface.
14. ❌ **Retry/backoff middleware — withdrawn.** The SDK already wires in `RetryHandler` via `MiddlewareFactory.getDefaultMiddlewareChain` with sensible defaults (3 retries, 3s base delay, honors `Retry-After` on 429/503/504). The original "no built-in retry" finding was outdated; both [src/graph/client.ts](src/graph/client.ts) and the README's Known-Limitations section now reflect the actual behavior.
15. ✅ **NOTICE attribution file** — [scripts/generate-notice.mjs](scripts/generate-notice.mjs) walks `package-lock.json`'s production tree and emits [NOTICE.md](NOTICE.md) (352 deps). Re-runnable via `npm run docs:notice`.
16. ✅ **Trademark + data-flow disclaimers in README** — added a "Data flow — what leaves your machine" section in Security and a Microsoft trademark footer.
17. ✅ **Logger redact list** — [src/util/logger.ts:3-14](src/util/logger.ts:3) now also redacts `id_token`, `idToken`, `client_assertion`, `clientAssertion`.
18. ✅ **Default-client warning** — [src/server.ts:35-42](src/server.ts:35) emits `logger.warn` at startup when `clientId === DEFAULT_PUBLIC_CLIENT_ID`.
19. ✅ **`isomorphic-fetch` dropped** — removed from [src/graph/client.ts](src/graph/client.ts) and [package.json](package.json); build/tests pass on Node 20+'s built-in fetch.
20. ✅ **Dead `clearLastDeviceCodePrompt` removed** — no longer exported from [src/auth/index.ts](src/auth/index.ts).

## Post-Implementation Notes

### Three latent bugs caught by the new fixture tests

The OOXML fixture suite ([test/ooxml.test.ts](test/ooxml.test.ts)) caught three bugs that had been in the codebase since v0.1 but couldn't have been found by the pre-existing test set, which had no OOXML coverage. All three were fixed in the OOXML refactor commit:

1. **`insertParagraphAt` off-by-one.** Documented as "after=0 → at the very top, after=N → after the Nth paragraph"; the implementation's `after * 2 + 2` formula inserted one paragraph too late at every index, so `after=0` actually landed after paragraph 1 and every other index was shifted. Fixed to splice after `<w:body>` for `after=0` and use `after * 2` for N≥1.

2. **`deleteSlide` regexes used `[^/]*`.** The `Type="..."` URL inside `<Relationship>` and `<Override>` elements contains `/` (e.g. `Type="http://schemas.openxmlformats.org/.../slide"`), so the regex stopped early and silently failed to remove the rel/Override entries — leaving dangling references in the output deck. Replaced all three with `[^>]*`.

3. **`extractText` / `extractSlideText` returned raw on-disk OOXML.** Any document containing `&`, `<`, `>`, `'`, or `"` came back with the entity form (`&amp;`, `&lt;`, etc.) instead of decoded plain text — broken for, e.g., almost any title with an apostrophe. Added a `decodeXmlEntities` helper to [src/ooxml/escape.ts](src/ooxml/escape.ts) and applied it in both extractors.

These reinforce the original review's call to flag the OOXML coverage gap at 🟠: those tools were producing incorrect output without throwing.

### One finding withdrawn after re-investigation

The System Architect note "no retry/backoff layer around the Graph client" was wrong. `Client.initWithMiddleware({ authProvider })` (which is what [src/graph/client.ts](src/graph/client.ts) calls) constructs the SDK's default chain via `MiddlewareFactory.getDefaultMiddlewareChain`, and that chain includes a `RetryHandler` configured with `RetryHandlerOptions` defaults: 3 retries, 3-second base delay, honors `Retry-After` on 429/503/504. The README's "No built-in retry on Graph 429/503" line had drifted from reality. Both the source comment and the Known-Limitations section have been corrected.

### One finding skipped after closer inspection

The Senior Developer note "inconsistent return shapes across mail tools" was reconsidered: send/reply/delete return `{ ok: true }` (no useful return value); create returns the entity (so the caller can reference it). The same pattern is used by `contacts_create`, `calendar_create_event`, etc. Changing it would *introduce* inconsistency, so left as-is.
